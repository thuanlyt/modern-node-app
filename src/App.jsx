...
const UPDATE_LOGS = [
  { version: '1.2.1', date: '27/03/2026', notes: ['Khôi phục chức năng Đổi màu Dot/Nền và cắt dây Middle Node', 'Thêm Animation pop-up mượt mà cho bảng Cài đặt', 'Hỗ trợ Click Outside (Bấm ra ngoài) để đóng nhanh Modal'] },
  { version: '1.2.0', date: '27/03/2026', notes: ['Tích hợp Cloud Database (Firebase)', 'Tính năng Quản trị Alias Link cho Admin', 'Bảo mật mã nguồn với Biến môi trường (.env)'] },
  { version: '1.1.0', date: '26/03/2026', notes: ['Kết nối 4 chiều (Trên, Dưới, Trái, Phải)', 'Hỗ trợ Word Wrap, tự động xuống dòng', 'Tối ưu tràn viền Mobile'] },
  { version: '1.0.0', date: '25/03/2026', notes: ['Khởi tạo dự án Modern Node VHD', 'Các tính năng thao tác Node cơ bản', 'Giao diện Glassmorphism Dark Mode'] }
];
...
function Flow() {
  ...
  useEffect(() => { 
    if (!isPublicView && isAppReady) {
      localStorage.setItem(STORAGE_KEY, JSON.stringify({ nodes, edges })); 
    }
  }, [nodes, edges, isAppReady]);

  useEffect(() => { localStorage.setItem(SETTINGS_KEY, JSON.stringify(settings)); }, [settings]);
  
  // eslint-disable-next-line react-hooks/exhaustive-deps
  useEffect(() => { 
    setEdgesState((eds) => eds.map(e => ({ ...e, type: settings.lineType }))); 
  }, [settings.lineType]);
  ...
  useEffect(() => {
    const handleBeforeInstall = (e) => { 
      e.preventDefault(); 
      setDeferredPrompt(e); 
      if (!installDismissed && !isPublicView) setShowInstallPrompt(true); 
    };
  ...
  const handleInstallApp = async () => {
    if (!deferredPrompt) { alert("Thiết bị đã cài App hoặc không hỗ trợ."); return; }
  ...
  const handleAddMiddleNode = () => {
    ...
    const newNode = {
      id: newNodeId, type: 'modernCard', position: { x: midX, y: midY },
      data: { title: '🔗 MIDDLE NODE', content: 'Node ở giữa', color: '#c084fc', nodeType: 'default', bgColor: '#1e293b' },
    };
  ...
  const updateNodeSpecificData = (key, value, closeMenu = true) => {
    setNodesState((nds) => nds.map((n) => n.id === menu.id ? { ...n, data: { ...n.data, [key]: value } } : n));
    if (closeMenu) setMenu(null);
  };
  ...
  const downloadImage = () => { 
    toPng(document.querySelector('.react-flow__viewport'), { backgroundColor: '#020617' }).then((dataUrl) => { 
      const a = document.createElement('a'); a.href = dataUrl; a.download = 'vhd_mindmap.png'; a.click(); 
    }); 
    setIsSettingsOpen(false); 
  };
  ...
  const handleExportJSON = () => { 
    ...
    const a = document.createElement('a'); a.href = dataStr; a.download = "vhd_nodes.json"; a.click(); 
    setIsSettingsOpen(false); 
  };
  ...
  const handleImportJSON = (e) => { 
    ...
    reader.onload = (event) => { 
      try { 
        ...
        if (parsed.nodes && parsed.edges) { setNodesState(parsed.nodes); setEdgesState(parsed.edges); fitView(); } 
  ...
  const handleChangeAdminPw = () => { 
    ...
    localStorage.setItem(ADMIN_PW_KEY, newPwInput.trim()); 
  ...
  const handleLoadLinkToCanvas = (alias) => { 
    setNodesState(publicLinksData[alias].nodes || []); 
    setEdgesState(publicLinksData[alias].edges || []); 
    setAliasInput(alias); 
  ...
  return (
    <div className="w-screen h-screen bg-slate-950 relative" ref={reactFlowWrapper} style={{ '--node-max-width': `${settings.nodeMaxWidth || 350}px`, '--show-color-dot': settings.showColorDot ? 'block' : 'none' }}>
      {/* CSS Animation siêu mượt cho Modal */}
      <style>{`
        @keyframes modalPop {
      ...
      >
        ...
        <Controls className="bg-slate-800 border-none shadow-lg rounded-lg fill-slate-200" />
        
        <Panel position="top-right" className="m-4 flex items-center gap-3">
          {isPublicView && <button onClick={() => window.location.href = '/'} className="px-5 h-12 bg-rose-600 rounded-full text-white font-bold text-sm shadow-2xl transition-transform hover:scale-105">🏠 Trang Chủ</button>}
          <button onClick={() => setIsSettingsOpen(true)} className="w-12 h-12 bg-slate-800/80 rounded-full shadow-2xl border border-slate-700 flex items-center justify-center hover:bg-slate-700 transition-colors group">
        ...
        <div className="absolute bottom-0 right-0 px-[4px] py-[2px] text-[10px] text-slate-500 bg-slate-800/50 rounded-tl pointer-events-none select-none z-50">
          Thuận LYT - VHD | v{APP_VERSION}
        </div>
      </ReactFlow>
      ...
      {isSettingsOpen && (
        <div 
          ...
          onClick={() => setIsSettingsOpen(false)} // Bấm ra ngoài để đóng
        >
          <div 
            className="bg-slate-900 border border-slate-700 shadow-2xl rounded-2xl p-6 w-full max-w-md max-h-[90vh] overflow-y-auto relative animate-modal-pop"
            onClick={(e) => e.stopPropagation()} // Chặn đóng khi bấm bên trong bảng
          >
            ...
            <div className="flex flex-col gap-4">
              ...
              <button onClick={() => window.open('https://github.com/thuanlyt/modern-node-vhd', '_blank')} className="w-full py-3 bg-slate-800 hover:bg-slate-700 rounded-xl text-sm text-slate-300 font-bold flex items-center justify-center gap-2 border border-slate-700 transition-colors">
                🐙 Xem Mã Nguồn (GitHub)
              </button>
      ...
      {isLogOpen && (
        <div 
          className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[120] flex items-center justify-center p-4 transition-opacity"
          onClick={() => setIsLogOpen(false)}
        >
          <div 
            className="bg-slate-900 border border-slate-700 shadow-2xl rounded-2xl p-6 w-full max-w-md max-h-[80vh] overflow-y-auto relative animate-modal-pop"
      ...
      {isAdminAuthOpen && (
        <div 
          className="fixed inset-0 bg-black/80 backdrop-blur-md z-[110] flex items-center justify-center p-4 transition-opacity"
          onClick={() => setIsAdminAuthOpen(false)}
        >
          <div 
            className="bg-slate-900 border border-emerald-500/30 shadow-2xl rounded-2xl p-6 w-full max-w-sm flex flex-col gap-4 animate-modal-pop"
            onClick={(e) => e.stopPropagation()}
          >
      ...
      {isAdminPanelOpen && (
        ...
        >
          <div 
            className="bg-slate-900 border border-emerald-500/50 shadow-2xl rounded-2xl p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto relative animate-modal-pop"
            onClick={(e) => e.stopPropagation()}
          >
